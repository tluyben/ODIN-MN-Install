...
ODIN Masternode installer arguments:
    ...
    -b --bootstrap            : Sync node using Bootstrap NOT SUPPORTED YET
    --no-bootstrap            : Don't use Bootstrap NOT SUPPORTED YET
...
# Set these to change the version of ODIN to install
...
apt-get install git zip dnsutils systemd -y > /dev/null 2>&1
...
echo "
 ...
 | the ODIN Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "odin"' && echo ""
sleep 1
...
cat > $USERHOME/.odin/odin.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
EOL
chmod 0600 $USERHOME/.odin/odin.conf
chown -R $USER:$USER $USERHOME/.odin
